extensions [shell]
globals [febioFileName orderFileName materialsFileName patchStrainFileName dataFileName]
patches-own [material strain]

to setup
  ca
  set febioFileName "testGridWithNL.feb"
  set orderFileName "orderedFEs.txt"
  set materialsFileName "patchMaterials.txt"
  set patchStrainFileName "patchStrain.txt"
  set dataFileName "dataFile.txt"
  
  ; Get FE order from left-to-right, top-to-bottom (becauase that's the order
  ; that patches will be updated). Ordered FEs are in "orderedFEs.txt".
  pythonWriteFEorder
  
  ; Add logfile output to FEBio simulation
  pythonAddLogFile
...
to go
  ; Update materials for each element
  updatePatchMaterial
  writePatchMaterial
  pythonUpdateFEBioMaterials
  
  ; Run FEBio
  FEBioSimulation
  
  ; Update patch strains
  pythonParseFEBioData
  updatePatchStrain
  colorStress
  tick
end
...
to updatePatchMaterial
;  ask patches
;  [
;    ;set material (random 2) + 1
;    ifelse strain > 5.5
;    [
;      set material 1
;    ]
...
;  ]
  ...
  ask patches
  [
    ifelse strain > mean ( [strain] of neighbors ) 
    [
      set material 1
    ]
    [
...
to pythonWriteFEorder
  let pcmd (word "python sortFEBioElements.py " febioFileName " " orderFileName)
  show (shell:exec "cmd" "/c" pcmd )
end

to pythonAddLogFile
  let pcmd (word "python addLoggingToFEBioFile.py " febioFileName " " dataFileName)
  show (shell:exec "cmd" "/c" pcmd )
end

to writePatchMaterial
  if file-exists? materialsFileName
  [
    file-delete materialsFileName  
  ]
  file-open materialsFileName  
...
to pythonUpdateFEBioMaterials
  let pcmd (word "python updateFEBioSim.py " orderFileName " " materialsFileName " " febioFileName)
  show (shell:exec "cmd" "/c" pcmd  )
end

to FEBioSimulation
  let pcmd (word ".\\FEBio1p8\\FEBio.exe -i " febioFileName)
  show pcmd
  show (shell:exec "cmd" "/c" pcmd )
end

to pythonParseFEBioData
  let pcmd (word "python parseFEBioData.py " dataFileName " " patchStrainFileName " " orderFileName)
  show (shell:exec "cmd" "/c" pcmd )
end

to updatePatchStrain
  file-open patchStrainFileName  
  foreach sort patches
  [
    ask ?
    [
      set strain file-read 
    ]
  ]
  file-close-all
end
...
to colorStress
  ask patches
  [
    set pcolor scale-color red strain 0 2
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
